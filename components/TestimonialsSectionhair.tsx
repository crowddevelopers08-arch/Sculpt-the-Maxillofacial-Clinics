...
const testimonials = [
  ...
  },
  {
    ...
    rating: 5,
    text: "I got my cosmelan and iv glutathione treatment done from Sculpt The Maxillofacial Clinic and and I am extremely happy with the results.Dr.Priyanka and her staff are so good that they explain all the steps of the procedure in detail.I will surely recommend Sculpt The Maxillofacial Clinic to all my friends and relatives.Thank you",
    image: "newsvcg.jpg?height=80&width=80",
  },
  {
    name: "V.Aishwarya",
...
export function TestimonialsSectionhair() {
    ...
    <section id="testimonials" ref={sectionRef} className="py-20 max-[426px]:py-5 lg:py-10 bg-white"style={{fontFamily: "'Outfit', sans-serif"}}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-[426px]:mb-8 mb-16">
          <h2
            className={`text-[40px] max-[426px]:text-[22px] max-[426px]:mb-3 sm:text-4xl font-bold text-[#0d0d0d] mb-6 font-sans ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => (
            <Card
              ...
              className={`bg-gradient-to-br from-[#0d0d0d] to-[#0d0d0d]/90 border-[#c99500]/20 hover:border-[#c99500]/40 transition-all duration-500 transform hover:scale-105 ${
              ...
              <CardContent className="p-8 max-[426px]:p-4">
                <div className="flex items-start space-x-4 mb-6">
                  <img
                    ...
                    className="w-16 h-16 rounded-full object-cover border-2 border-[#c99500]/30"
                  />
                  <div className="flex-1">
                    <h4 className="text-xl max-[426px]:text-[16px] font-bold text-white mb-1 font-sans">{testimonial.name}</h4>
                    <div className="flex items-center space-x-1">
                      {[...Array(testimonial.rating)].map((_, i) => (
                        <Star key={i} className="w-4 h-4 fill-[#c99500] text-[#c99500]" />
                      ))}
                    </div>
                  </div>
                  <Quote className="w-8 h-8 text-[#c99500]/30 flex-shrink-0" />
                </div>
        ...
        <div className={`text-center mt-16 max-[426px]:mt-8 ${isVisible ? "animate-fade-in-up animation-delay-800" : "opacity-0"}`}>
          <div className="inline-flex items-center space-x-4 bg-black/5 rounded-full px-8 py-4">
            <div className="flex items-center space-x-2">
              <Star className="w-6 h-6 fill-[#c99500] text-[#c99500]" />
              <span className="text-2xl max-[426px]:text-[22px] font-bold text-[#0d0d0d]">4.9/5</span>
            </div>
